# Set Project Name
PROJECT_NAME := socket_ekf_stockholm_zrd
...
erase-flash-settings:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s es 0x74000 0x2000
...
flash-orig-write:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s -m we 0 $(BIN_PATH)/orig/socket_ekf_stockholm_orig.bin
	
flash-orig-read:
	@python3 $(TOOLS_PATH)/TlsrPgm.py -p$(DOWNLOAD_PORT) -z11 -a 100 -s -m rf 0 0x100000 socket_ekf_stockholm_orig.bin
...
$(BIN_FILE): $(ELF_FILE)
	...
	@python3 $(MAKE_OTA) -t $(PROJECT_NAME) -m 4417 -i 897 -v0x1111114b -s "Slacky-DIY OTA" $(BIN_PATH)/$(PROJECT_NAME)_$(VERSION_RELEASE).$(VERSION_BUILD).bin   
	@echo ' '
	@echo 'Finished building: $@'
	@echo ' '
